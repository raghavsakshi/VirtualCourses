...
function ViewCourses() {
  ...
  const fetchCourseData = useCallback(async () => {
    ...
    if (course) {
      ...
      console.log('Course found in Redux store:', course)
    } else {
      // If not in store, fetch from backend
      try {
        console.log('Course not in Redux, fetching from backend...')
        const result = await axios.get(serverUrl + `/api/course/getCourse/${courseId}`,
          { withCredentials: true })
        dispatch(setSelectedCourse(result.data))
        console.log('Course fetched from backend:', result.data)
      } catch (error) {
  ...
  const checkEnrollment = useCallback(() => {
    const verify = userData?.enrolledCourses?.some(c => (typeof c === 'string' ? c : c._id).toString()
      === courseId?.toString())
    if (verify) {
      setIsEnrolled(true)
    }
          ...
          <div className='flex-1 space-y-2  mt-[20px]'>
            ...
            <div className='flex items-start flex-col justify-between'>
              <div className='text-yellow-500 font-medium flex gap-2'>
                <span className='flex items-center justify-start gap-2'>< FaStar />{avgRating}</span>
 ...
 cursor-pointer'  onClick={() => navigate(`/viewlecture/${courseId}`)}>Watch Now</button>}
        ...
        <div className='flex flex-col md:flex-row gap-6'>
          <div className='bg-white w-full md:w-2/5 p-6 rounded-2xl shadow-lg border border-gray-200'>
            <h2 className='text-xl font-semibold mb-1 text-gray-800'> Course Curriculum</h2>
    ...
    transition-all duration-200 text-left ${lecture.isPreviewFree ? "hover:bg-gray-100 cursor-pointer border-gray-300" :
                      "cursor-not-allowed opacity-600 border-gray-200"} ${selectedLecture?.lectureTitle === lecture?.lectureTitle ?
                        "bg-gray-100 border-gray-400"
                        : ""}`}>
                  <span className='text-lg text-gray-700'>   {lecture.isPreviewFree ? <FaCirclePlay /> : <FaLock />}</span>
          ...
          <div className='bg-white w-full md:w-3/5 p-6 rounded-2xl overflow-hidden mb-4 bg-black flex items-center justify-center'>
            <div className='aspect-video w-full rounded-lg overflow-hidden mb-4 bg-black flex items-center 
  justify-center'>
        ...
        <div className='mt-8 border-t pt-6'>
          ...
          <div className='mb-4'>
            <div className='flex gap-1 mb-2'>
              {
                [1, 2, 3, 4, 5].map((star) => (
                  <FaStar key={star} onClick={() => setRating(star)} className={star <= rating ? "text-amber-300" : "text-gray-300"} />
                ))
              }
            </div>
            <textarea onChange={(e) => setcomment(e.target.value)} value={comment} className='w-full border border-gray-300 rounded-lg p-2'
              placeholder='Write your review here...' rows={3}></textarea>
        ...
        <div className='flex items-center gap-4 pt-4 border-t'>
          {creatorData?.photoUrl ? <img src={creatorData?.photoUrl} alt="" className='w-16 h-16 rounded-full object-cover border-1 border-gray-200 ' /> :
        ...
        <div className='w-full transition-all duration-300 py-[20px] flex items-center justify-center
