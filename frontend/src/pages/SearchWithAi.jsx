...
function SearchWithAi() {
    ...
    from-black to-gray-900 text-white flex flex-col items-center px-4 py-1 pt-24'>
            <Nav />
        ...
        text-center relative'>
                ...
                <h1 className='text-2xl sm:text-3xl font-bold text-gray-600 mb-6 flex items-center justify-center gap-2'>
                    <img src={ai} alt="" className='w-8 h-8 sm:w-[30px] sm:h-[30px]' /> Search with <span className='text-[#CB99C7] '>Ai</span>
                </h1>
    ...
    text-white placeholder-gray-400 focus:outline-none text-sm sm:text-base'placeholder='What do you want to learn?(e.g. AI,MERN,Cloud...'
                        onChange={(e) => setInput(e.target.value)} value={input} />
                    {input && <button className=' absolute right-14 bg-white rounded-full sm:right-16  '>
            ...
            {recommedations.length > 0 ? (
                <div className='w-full max-w-6xl mt-12 px-2 sm:px-4'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8'>
                        {
                            recommedations?.map((course, index) => (
                                <div key={index} className='bg-white text-black p-5 rounded-2xl shadow-md hover:shadow-indigo-500/30 
                           transtion-all duration-200  border border-gray-200 cursor-pointer hover:bg-gray-200'
                ...
                listening ? <h1 className='text-center text-xl sm:text-2xl mt-10 text-gray-400'>
                    Listening ...</h1> : <h1 className='text-center text-xl sm:text-2xl mt-10 text-gray-400'>
                    No Courses Found Yet.</h1>
            )}
        </div>
    )
}
